...
(
    ...
    password   VARCHAR(255) NULL COMMENT '암호화된 비밀번호 (OAuth2 유저는 NULL)',
    nickname   VARCHAR(255) NOT NULL COMMENT '회원 닉네임',
    role       VARCHAR(255) NOT NULL DEFAULT 'USER' COMMENT '회원 권한 (USER, ADMIN)',
    status     VARCHAR(255) NOT NULL DEFAULT 'INACTIVE' COMMENT '회원 상태 (INACTIVE, ACTIVE, DELETED, BLOCKED)',
...
COMMENT ON TABLE members IS '회원 테이블';

CREATE TABLE oauth_connections
(
    oauth_connection_id BIGINT       NOT NULL AUTO_INCREMENT COMMENT 'OAuth 연결 고유 식별자',
...
COMMENT ON TABLE oauth_connections IS 'OAuth 연결 테이블';
...
(
    ...
    like_count    INTEGER      NOT NULL DEFAULT 0 COMMENT '추천수',
    comment_count INTEGER      NOT NULL DEFAULT 0 COMMENT '댓글 수',
...
(
    ...
    post_id    BIGINT     NOT NULL COMMENT '좋아요를 받은 게시글 ID',
    created_at DATETIME   NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성 일시',
    updated_at DATETIME   NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '수정 일시',
...
(
    ...
    post_id    BIGINT     NOT NULL COMMENT '댓글이 작성된 게시글 ID',
    member_id  BINARY(16) NOT NULL COMMENT '작성한 회원 ID',
...
(
    member_id  BINARY(16) NOT NULL COMMENT '좋아요를 누른 회원 ID',
    comment_id BIGINT     NOT NULL COMMENT '좋아요를 받은 댓글 ID',
    created_at DATETIME   NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성 일시',
    updated_at DATETIME   NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '수정 일시',
